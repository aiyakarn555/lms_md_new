...
class MsTeamsInstitutionController extends Controller
{
	/**
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['MsTeamsInstitution']))
		{

			$time = date("dmYHis");
			$model->attributes=$_POST['MsTeamsInstitution'];
			...
			if($model->validate() && $statusCheckTime->status == "true")
			{
               	...
               	if($type == "msteam"){
               		...
               		$start_date = date_format(date_create($model->start_date." ".$_POST["MsTeamsInstitution"]["time_start_date"]), "Y-m-d H:i:s");
               		$end_date = date_format(date_create($model->end_date." ".$_POST["MsTeams"]["time_end_date"]), "Y-m-d H:i:s");
               		$data = array();
               		$data['topic'] 		= $_POST["MsTeamsInstitution"]["name_ms_teams"];
               		$data['start_date'] = $start_date;
               		$data['end_date'] = $end_date;
               		$data['duration'] 	= $_POST["MsTeamsInstitution"]["duration"];
               		$data['type'] 		= 8;
                ...
                if($model->save())
                {
					...
					$model->type_ms_teams = 2; // 1 = ออนไลน์ 2 = ออนไลน์(สถาบัน)
					$model->save(false);
                	...
                	{
						/////////// SAVE IMAGE //////////
						$model = MsTeams::model()->findByPk($model->id);
                		Yush::init($model);
		...
		if(isset($_POST['MsTeamsInstitution']))
		{
			...
			if($model->validate())
			{
				...
				if($model->save())
				{
					...
					if( !isset($_POST['MsTeamsInstitution'][intro_video]) ){
	...
	{
		$model=new MsTeamsInstitution('search');
	...
	{
		$model=MsTeamsInstitution::model()->findByPk($id);
		if($model===null)
