...
class OrgBuController extends Controller
{
	/**
...
//	}
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['OrgBu']))
		{
			$Validation =  $this->formValidation($_POST['OrgBu']['code']);
			if(count($Validation) == 0){
				$model->attributes=$_POST['OrgBu'];
				$model->active = 'y';
				if($model->save()){
					$criteria = new CDbCriteria;
					$criteria->compare('id',$_POST['OrgBu']['group_bu_id']);
					$OrgGroupBu = OrgGroupBu::model()->find($criteria);

					$criteria = new CDbCriteria;
					$criteria->compare('group_bu_id',$_POST['OrgBu']['group_bu_id']);
					$criteria->compare('code',$OrgGroupBu->code);
					$criteria->compare('active','y');
					$parent = OrgChart::model()->find($criteria);

					$OrgChart = new OrgChart;
					$OrgChart->bu_id = $model->id;
					$OrgChart->group_bu_id = $_POST['OrgBu']['group_bu_id'];
					$OrgChart->code =  $_POST['OrgBu']['code'];
					$OrgChart->title =  $_POST['OrgBu']['name'];
					$OrgChart->level =  3;
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['OrgBu']))
		{	
			$Validation =  $this->formValidation($_POST['OrgBu']['code'], $id, $old_code);
			if(count($Validation) == 0){
				$model->attributes=$_POST['OrgBu'];
				if($model->save()){
					...
					$criteria = new CDbCriteria;
					$criteria->compare('bu_id',$model->id);
					...
					$criteria->compare('bu_id',$model->id);
					$criteria->compare('active','y');
					$OrgChartBus = OrgChart::model()->findAll($criteria);
					foreach($OrgChartBus as $OrgChartBu){
						$criteria = new CDbCriteria;
						$criteria->compare('id',$OrgChartBu->id);
						$ChartBu = OrgChart::model()->find($criteria);
						$ChartBu->group_bu_id =  $OrgChart->group_bu_id;
						$ChartBu->save();
					}
					if(Yii::app()->user->id){
						Helpers::lib()->getControllerActionId();
	...
	{
		$model=new OrgBu('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['OrgBu']))
			$model->attributes=$_GET['OrgBu'];
