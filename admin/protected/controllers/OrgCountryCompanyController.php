...
class OrgCountryCompanyController extends Controller
{
	/**
...
//	}
	...
	{
		$model = new OrgCountryCompany;
		// Uncomment the following line if AJAX validation is needed
		if(isset($_POST['OrgCountryCompany']))
		{
			$Validation =  $this->formValidation($_POST['OrgCountryCompany']['code']);
			if(count($Validation) == 0){
				$model->attributes=$_POST['OrgCountryCompany'];
				$model->active = 'y';
				if($model->save()){
					$criteria = new CDbCriteria;
					$criteria->compare('id',$_POST['OrgCountryCompany']['country_id']);
					$OrgCountry = OrgCountry::model()->find($criteria);

					$criteria = new CDbCriteria;
					$criteria->compare('country_id',$_POST['OrgCountryCompany']['country_id']);
					$criteria->compare('code',$OrgCountry->code);
					$criteria->compare('active','y');
					$parent = OrgChart::model()->find($criteria);

					$OrgChart = new OrgChart;
					$OrgChart->company_id = $model->id;
					$OrgChart->country_id = $_POST['OrgCountryCompany']['country_id'];
					$OrgChart->code =  $_POST['OrgCountryCompany']['code'];
					$OrgChart->title =  $_POST['OrgCountryCompany']['name'];
					$OrgChart->level =  3;
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['OrgCountryCompany']))
		{	
			$Validation =  $this->formValidation($_POST['OrgCountryCompany']['code'], $id, $old_code);
			if(count($Validation) == 0){
				$model->attributes=$_POST['OrgCountryCompany'];
				if($model->save()){
					...
					$criteria = new CDbCriteria;
					$criteria->compare('company_id',$model->id);
					...
					$criteria->compare('company_id',$model->id);
					$criteria->compare('active','y');
					$OrgChartCompanys = OrgChart::model()->findAll($criteria);
					foreach($OrgChartCompanys as $OrgChartCompany){
						$criteria = new CDbCriteria;
						$criteria->compare('id',$OrgChartCompany->id);
						$ChartCompany = OrgChart::model()->find($criteria);
						$ChartCompany->country_id =  $OrgChart->country_id;
						$ChartCompany->save();
					}
					if(Yii::app()->user->id){
						Helpers::lib()->getControllerActionId();
	...
	{
		$model=new OrgCountryCompany('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['OrgCountryCompany']))
			$model->attributes=$_GET['OrgCountryCompany'];
...
//	}
	...
	{
		$model=OrgCountryCompany::model()->findByPk($id);
		if($model===null)
