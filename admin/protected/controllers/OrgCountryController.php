...
//	}
	...
	public function actionCreate()
	{
		...
		{
			$Validation =  $this->formValidation($_POST['OrgCountry']['code']);
			if(count($Validation) == 0){
				$model->attributes=$_POST['OrgCountry'];
				$model->active = 'y';
				if($model->save()){

					$OrgChart = new OrgChart;
					$OrgChart->country_id = $model->id;
					$OrgChart->code =  $_POST['OrgCountry']['code'];
					$OrgChart->title =  $_POST['OrgCountry']['name'];
					$OrgChart->level =  2;
					$OrgChart->parent_id =  2;
					$OrgChart->active = 'y';
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['OrgCountry']))
		{	
			$Validation =  $this->formValidation($_POST['OrgCountry']['code'], $id, $old_code);
			if(count($Validation) == 0){
				$model->attributes=$_POST['OrgCountry'];
				if($model->save()){

					$criteria = new CDbCriteria;
					$criteria->compare('country_id',$model->id);
	...
	{
		$model=new OrgCountry('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['OrgCountry']))
			$model->attributes=$_GET['OrgCountry'];
...
//	}
	...
	{
		$model=OrgCountry::model()->findByPk($id);
		if($model===null)
