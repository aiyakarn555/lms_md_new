...
class OrgCountrySectionController extends Controller
{
	/**
...
//	}
	...
	{
		$model = new OrgCountrySection;
		// Uncomment the following line if AJAX validation is needed
		if(isset($_POST['OrgCountrySection']))
		{
			$Validation =  $this->formValidation($_POST['OrgCountrySection']['code']);
			if(count($Validation) == 0){
				$model->attributes=$_POST['OrgCountrySection'];
				$model->active = 'y';
				if($model->save()){
					$criteria = new CDbCriteria;
					$criteria->compare('id',$_POST['OrgCountrySection']['division_id']);
					...
					$criteria = new CDbCriteria;
					$criteria->compare('division_country_id',$_POST['OrgCountrySection']['division_id']);
					$criteria->compare('code',$OrgCountryDivision->code);
					$criteria->compare('active','y');
					$parent = OrgChart::model()->find($criteria);

					$OrgChart = new OrgChart;
					$OrgChart->section_country_id = $model->id;
					...
					$OrgChart->code =  $_POST['OrgCountrySection']['code'];
					$OrgChart->title =  $_POST['OrgCountrySection']['name'];
					$OrgChart->level =  6;
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['OrgCountrySection']))
		{	
			$Validation =  $this->formValidation($_POST['OrgCountrySection']['code'],$id, $old_code);
			if(count($Validation) == 0){
				$model->attributes=$_POST['OrgCountrySection'];
				if($model->save()){
					$criteria = new CDbCriteria;
					...
					foreach($OrgChartSections as $OrgChartSection){
						...
						$ChartCountrySection->division_country_id = $OrgCountryDivision->id;
						$ChartCountrySection->department_country_id = $OrgCountryDepartment->id;
						$ChartCountrySection->company_id = $OrgCountryCompany->id;
						$ChartCountrySection->country_id = $OrgCountryCompany->country_id;
						$ChartSection->save();
					}
	...
	{
		$model=new OrgCountrySection('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['OrgCountrySection']))
			$model->attributes=$_GET['OrgCountrySection'];
...
//	}
	...
	{
		$model=OrgCountrySection::model()->findByPk($id);
		if($model===null)
	...
	protected function formValidation($code, $id = null, $old_code = null)
	{
		...
		$criteria->compare('code',$old_code);
		$criteria->compare('active','y');
		$criteria->compare('section_country_id',$id);
