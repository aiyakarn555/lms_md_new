...
class OrgCountryTeamController extends Controller
{
	/**
...
//	}
	...
	{
		$model = new OrgCountryTeam;
		// Uncomment the following line if AJAX validation is needed
		if(isset($_POST['OrgCountryTeam']))
		{
			$Validation =  $this->formValidation($_POST['OrgCountryTeam']['code']);
			if(count($Validation) == 0){
				$model->attributes=$_POST['OrgCountryTeam'];
				$model->active = 'y';
				if($model->save()){
					$criteria = new CDbCriteria;
					$criteria->compare('id',$_POST['OrgCountryTeam']['section_id']);
					$criteria->compare('active','y');
					$OrgCountrySection = OrgCountrySection::model()->find($criteria);

					$criteria = new CDbCriteria;
					$criteria->compare('id',$OrgCountrySection->division_id);
					$criteria->compare('active','y');
					$OrgCountryDivision = OrgCountryDivision::model()->find($criteria);

					$criteria = new CDbCriteria;
					$criteria->compare('id',$OrgCountryDivision->department_id);
					$criteria->compare('active','y');
					...
					$criteria = new CDbCriteria;
					$criteria->compare('section_country_id',$_POST['OrgCountryTeam']['section_id']);
					$criteria->compare('code',$OrgCountrySection->code);
					$criteria->compare('active','y');
					$parent = OrgChart::model()->find($criteria);

					$OrgChart = new OrgChart;
					$OrgChart->team_id = $model->id;
					$OrgChart->section_country_id = $OrgCountrySection->id;
					$OrgChart->division_country_id = $OrgCountryDivision->id;
					...
					$OrgChart->code =  $_POST['OrgCountryTeam']['code'];
					$OrgChart->title =  $_POST['OrgCountryTeam']['name'];
					$OrgChart->level =  7;
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['OrgCountryTeam']))
		{	
			$Validation =  $this->formValidation($_POST['OrgCountryTeam']['code'], $id, $old_code);
			if(count($Validation) == 0){
				$model->attributes=$_POST['OrgCountryTeam'];
				...
				if($model->save()){
					...
					$criteria->compare('team_id',$model->id);
					$criteria->compare('active','y');
					$OrgChartteams = OrgChart::model()->findAll($criteria);
					foreach($OrgChartteams as $OrgChartteam){
						$criteria = new CDbCriteria;
						$criteria->compare('id',$OrgChartteam->id);
						$ChartSubSection = OrgChart::model()->find($criteria);
						$ChartSubSection->section_country_id = $OrgCountrySection->id;
						$ChartSubSection->division_country_id = $OrgCountryDivision->id;
						$ChartSubSection->department_country_id = $OrgCountryDepartment->id;
						$ChartSubSection->company_id = $OrgCountryCompany->id;
						$ChartSubSection->country_id = $OrgCountryCompany->country_id;
	...
	public function actionDelete($id)
	{
		...
		$criteria = new CDbCriteria;
		$criteria->compare('section_country_id',$model->id);
	...
	{
		$model=new OrgCountryTeam('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['OrgCountryTeam']))
			$model->attributes=$_GET['OrgCountryTeam'];
...
//	}
	...
	{
		$model=OrgCountryTeam::model()->findByPk($id);
		if($model===null)
