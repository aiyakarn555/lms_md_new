...
//	}
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['OrgDepartment']))
		{
			$Validation =  $this->formValidation($_POST['OrgDepartment']['code']);
			if(count($Validation) == 0){
				$model->attributes=$_POST['OrgDepartment'];
				$model->active = 'y';
				if($model->save()){

					$criteria = new CDbCriteria;
					$criteria->compare('id',$_POST['OrgDepartment']['bu_id']);
					$criteria->compare('active','y');
					$OrgBu = OrgBu::model()->find($criteria);

					$criteria = new CDbCriteria;
					$criteria->compare('bu_id',$_POST['OrgDepartment']['bu_id']);
					$criteria->compare('code',$OrgBu->code);
					$criteria->compare('active','y');
					$parent = OrgChart::model()->find($criteria);
					
					$OrgChart = new OrgChart;
					$OrgChart->department_id = $model->id;
					$OrgChart->group_bu_id = $OrgBu->group_bu_id;
					$OrgChart->bu_id = $_POST['OrgDepartment']['bu_id'];
					$OrgChart->code =  $_POST['OrgDepartment']['code'];
					$OrgChart->title =  $_POST['OrgDepartment']['name'];
					$OrgChart->level =  4;
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['OrgDepartment']))
		{	
			$Validation =  $this->formValidation($_POST['OrgDepartment']['code'], $id, $old_code);
			if(count($Validation) == 0){
				$model->attributes=$_POST['OrgDepartment'];
				...
				if($model->save()){
					...
					$criteria = new CDbCriteria;
					$criteria->compare('department_id',$model->id);
					...
					foreach($OrgChartDepartments as $OrgChartDepartment){
						...
						$ChartDepartment->group_bu_id =  $OrgChart->group_bu_id;
						$ChartDepartment->bu_id =  $OrgChart->bu_id;
	...
	{
		$model=new OrgDepartment('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['OrgDepartment']))
			$model->attributes=$_GET['OrgDepartment'];
...
//	}
	...
	{
		$model=OrgDepartment::model()->findByPk($id);
		if($model===null)
