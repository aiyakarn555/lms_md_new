...
//		$modelChoice = new Choice;
		...
		if(isset($_POST['Question']))
		{
			foreach($_POST['Question'] as $key => $question) {
				...
				$questionModel = new QuestionMsTeams();
    ...
    {
        $model=new QuestionMsTeams('import');

        if(isset($_POST['QuestionMsTeams']))
        {      
            $phpExcelPath = Yii::getPathOfAlias('ext.phpexcel.Classes');
            include($phpExcelPath . DIRECTORY_SEPARATOR . 'PHPExcel.php');

            $model->attributes=$_POST['QuestionMsTeams'];
...
//				echo '</pre><hr />';
                ...
                $this->redirect(array('GrouptestingMsTeams/index'));
            //}
        }
	...
	{
		$modelQues = new QuestionMsTeams;
		$modelChoice = new ChoiceMsTeams;
	...
	public function actionShowPoint()
	{
		if(isset($_POST['Count']))
		{
			...
			for($i=0; $i<(int)$count; $i++)
			{
			    ...
			    $Question = QuestionMsTeams::model()->find($criteria);

				$scoreGood[$i] = 0; //Good
				$scoreWrong[$i] = 0; //Wrong
				$scoreChoice[$i] = 0;
				$countChoice = ChoiceMsTeams::model()->findAll("ques_id=:ques_id", array(
					...
					if(($questionType != 'textarea') && ($questionType != 'dropdown')) {
						...
						if(count($update_ids) > 0){
							foreach ($update_ids as $choice_id) {
								...
								$choiceModel = ChoiceMsTeams::model()->findByPk($choice_id);
//								$choiceModel->ques_id = $questionModel->ques_id;
								$choiceModel->choice_detail = $choiceTitle[$choice_id];
//								$choiceModel->choice_type = $questionType;
								if (in_array($choice_id, $answerValidCheckArray)) {
						...
						if(count($delete_ids) > 0){
							foreach ($delete_ids as $choice_id) {
								if ($choiceModel = ChoiceMsTeams::model()->findByPk($choice_id)) {
									$choiceModel->delete();
								}
							}
						}
	...
	public function actionIndex($id)
	{
		...
		if(isset($id))
		{
			...
			$model = new QuestionMsTeams('search');
			$model->unsetAttributes();

			if(isset($_GET['QuestionMsTeams']))
				$model->attributes = $_GET['QuestionMsTeams'];
	...
	{		
		$grouptesting = GrouptestingMsTeams::model()->findByPk($id);
		$question = QuestionMsTeams::model()->findAll([
	...
	{
		$model=QuestionMsTeams::model()->questioncheck()->findByPk($id);
		if($model===null)
			throw new CHttpException(404,'The requested page does not exist.');
		return $model;
	}
