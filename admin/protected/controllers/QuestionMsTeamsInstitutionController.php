...
class QuestionMsTeamsInstitutionController extends Controller
{
    	...
    	return array(
            array('allow',  // allow all users to perform 'index' and 'view' actions
            	'actions' => array('index', 'view','MultiDelete', 'Export'
            		,'import','create','update','delete' //เพิ่มชั่วคราว
            	),
            	'users' => array('*'),
            	),
            array('allow',
...
//		$modelChoice = new Choice;
		...
		if(isset($_POST['Question']))
		{
			foreach($_POST['Question'] as $key => $question) {
				...
				$questionModel = new QuestionMsTeamsInstitution();
					...
					if(($questionType != 'textarea') && ($questionType != 'dropdown')) {
						...
						if (count($choiceTitle) > 0) {
							foreach ($choiceTitle as $choiceKey => $choice) {
								$choiceModel = new ChoiceMsTeamsInstitution();
    ...
    {
        $model=new QuestionMsTeamsInstitution('import');

        if(isset($_POST['QuestionMsTeamsInstitution']))
        {      
            $phpExcelPath = Yii::getPathOfAlias('ext.phpexcel.Classes');
            include($phpExcelPath . DIRECTORY_SEPARATOR . 'PHPExcel.php');

            $model->attributes=$_POST['QuestionMsTeamsInstitution'];
...
//				echo '</pre><hr />';
                ...
                $this->redirect(array('GrouptestingMsTeamsInstitution/index'));
            //}
        }
	...
	{
		$modelQues = new QuestionMsTeamsInstitution;
		$modelChoice = new ChoiceMsTeamsInstitution;
		...
		for($i=0; $i<=(int)$count; $i++)
		{
			...
			{
				/***** SAVE *****/
				$SaveQues = new QuestionMsTeamsInstitution;
	...
	public function actionShowPoint()
	{
		if(isset($_POST['Count']))
		{
			...
			for($i=0; $i<(int)$count; $i++)
			{
			    ...
			    $Question = QuestionMsTeamsInstitution::model()->find($criteria);

				$scoreGood[$i] = 0; //Good
				$scoreWrong[$i] = 0; //Wrong
				$scoreChoice[$i] = 0;
				$countChoice = ChoiceMsTeamsInstitution::model()->findAll("ques_id=:ques_id", array(
					...
					if(($questionType != 'textarea') && ($questionType != 'dropdown')) {
						...
						if(count($update_ids) > 0){
							foreach ($update_ids as $choice_id) {
								...
								$choiceModel = ChoiceMsTeamsInstitution::model()->findByPk($choice_id);
//								$choiceModel->ques_id = $questionModel->ques_id;
								$choiceModel->choice_detail = $choiceTitle[$choice_id];
//								$choiceModel->choice_type = $questionType;
								if (in_array($choice_id, $answerValidCheckArray)) {
						...
						if(count($delete_ids) > 0){
							foreach ($delete_ids as $choice_id) {
								if ($choiceModel = ChoiceMsTeamsInstitution::model()->findByPk($choice_id)) {
									$choiceModel->delete();
								}
							}
						}
	...
	public function actionIndex($id)
	{
		...
		if(isset($id))
		{
			...
			$model = new QuestionMsTeamsInstitution('search');
			$model->unsetAttributes();

			if(isset($_GET['QuestionMsTeamsInstitution']))
				$model->attributes = $_GET['QuestionMsTeamsInstitution'];
	...
	{		
		$grouptesting = GrouptestingMsTeamsInstitution::model()->findByPk($id);
		$question = QuestionMsTeamsInstitution::model()->findAll([
	...
	{
		$model=QuestionMsTeamsInstitution::model()->questioncheck()->findByPk($id);
		if($model===null)
			throw new CHttpException(404,'The requested page does not exist.');
		return $model;
	}
	...
	{
		$model=GrouptestingMsTeamsInstitution::model()->findByPk($id);
		if($model===null)
			throw new CHttpException(404,'The requested page does not exist.');
		return $model;
	}
