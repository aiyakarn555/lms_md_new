<?php
$titleName = 'ภาพถ่ายห้องเรียนศึกษาด้วยตนเอง';
...
<div class="innerLR">
            ...
            <div class="search-form">
                <div class="wide form" style="padding-top:6px;">
                    <form id="SearchFormAjax" action="" method="get">
                        <div class="row">
                            <label>ห้องเรียนศึกษาด้วยตนเอง</label>
                            <select class="span6 chosen" name="course_id" id="course_id">
                                ...
                                ?>
                                        <option <?= ($_GET['course_id'] == $EachCourse->id ? 'selected="selected"' : '') ?> value="<?= $EachCourse->id ?>"><?= (( $EachCourse->course_md_code != "" && $EachCourse->course_md_code != null )?$EachCourse->course_md_code: "ไม่พบรหัส")." : ".$EachCourse->course_title ?></option>
                                <?php  }
                                } ?>
                            </select>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <select class="span6 chosen" name="gen_id" id="gen_id">
                                <?php if( $_GET['course_id'] == null && $_GET['gen_id'] == null) { ?>
                                    <option value="">ทั้งหมด</option>
                                <?php }else{ ?>
                                    <?php 
                                        $criteria = new CDbCriteria;
    ...
    <div class="widget" style="margin-top: -1px;">
        <div class="widget-head">
            <h4 class="heading glyphicons show_thumbnails_with_lines"><i></i> <?php echo $titleName; ?> <b style="color: red">กรุณาเลือกห้องเรียนศึกษาด้วยตนเอง แล้วค้นหา</b></h4>
        </div>
                ...
                <table class="table table-main">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                              <?= $value_da->course->course_title ?>
                            </td>
                            <td class="text-center">
                                <a href="<?php echo $this->createUrl('/CaptureScreen/ExportViewTheory/'.$value_da->user_id.'?course_id='.$value_da->course_id); ?>" class="btn-pic btn btn-primary">
                                    <span class="glyphicon glyphicon-eye-open"></span> รูปภาพ
                                </a>
                            </td>
                            <td class="text-center">
                                <a href="<?= $this->createUrl('/CaptureScreen/GenSingleExcelCaptureScreenTheory/'.$value_da->user_id.'?course_id='.$value_da->course_id) ?>" class="btn-excel btn ">
    ...
    <div class="mt-2">
        <?php if(count($data) > 0) { 
                $hrefImageAll = $this->createUrl('/CaptureScreen/ExportViewAllTheory/'.$value_da->course_id);
                $hrefExcelAll = $this->createUrl('/CaptureScreen/GenAllExcelCaptureScreenTheory') ;
                $action = '';
        }else{
