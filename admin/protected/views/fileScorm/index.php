		...
		<div class="widget-body">
			<div class="separator bottom form-inline small">
				<span class="pull-left" style="margin-left: 10px;">
					<?php
					$modelCheck = FileScorm::model()->findByAttributes(array("lesson_id" => $id,'active'=>'y'));
					if (empty($modelCheck)) { ?>
						<a class="btn btn-primary btn-icon glyphicons circle_plus"
						href="<?php echo Yii::app()->createUrl("/FileScorm/create",array("id"=>$id))?>"><i></i> เพิ่ม Scorm</a>
					<?php }
					?>
						...
						array(
							...
							'value' => function($val) {
								...
								foreach ($lang as $key => $value) {
									if ($key == 0) {
										$menu = FileScorm::model()->findByAttributes(array("lang_id" => $value->id,'id'=> $val->id,'active'=>'y'));
										$menu_lesson = Lesson::model()->findByAttributes(array("lang_id" => $value->id,'id'=> $val->lesson_id,'active'=>'y'));
									}else{
										$menu = FileScorm::model()->findByAttributes(array("lang_id" => $value->id,'parent_id'=> $val->id,'active'=>'y'));
										$menu_lesson = Lesson::model()->findByAttributes(array("lang_id" => $value->id,'parent_id'=> $val->lesson_id,'active'=>'y'));
									}
									if ($menu_lesson) {
										$str = ' (เพิ่ม)';
										$link = array("/FileScorm/create","lang_id"=>$value->id,"parent_id"=>$val->id);
										$class = "btn btn-icon";
										if($menu){
											$id = $menu ? $menu->id : $val->id;
											$str = ' (แก้ไข)';
											$class = "btn btn-success btn-icon";
											$link = array("/FileScorm/update","id"=>$id,"lang_id"=>$value->id,"parent_id"=>$val->id);
										}
