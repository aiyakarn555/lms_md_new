					 ...
					 //    ),
						array(
					        ...
					        'value' => function($data){
					        	if($data->type == 'pdf'){
					        		$str = 'จัดการ PDF';
					        		$count = ($data->getCountFilePDFLang($data->id));

					        		$path = 'FilePdf/index';
					        	} else if($data->type == 'vdo') {
					        		$str = 'จัดการ VDO';
					        		$count = ($data->getCountFileLang($data->id));
					        		$path = 'File/index';
					        	} else if($data->type == 'scorm'){
					        		$str = 'ไฟล์ Scorm';
					        		$count = ($data->getCountFileScormLang($data->id));
					        		...
					        		$str = 'ไฟล์ Audio';
					        		$count = ($data->getCountFileAudioLang($data->id));
					        		$path = 'FileAudio/index';
					        	} else if($data->type == 'youtube') {
					        		$str = 'จัดการ Youtube';
					        		$count = count($data->files);
					        		$path = 'FileYoutube/index';
					        	}
						...
						array(
							...
							'value' => function($val) {
								...
								foreach ($lang as $key => $value) {
									if ($key == 0) {
										$menu = Lesson::model()->findByAttributes(array("lang_id" => $value->id,'id'=> $val->id,'active'=>'y'));
									}else{
										$menu = Lesson::model()->findByAttributes(array("lang_id" => $value->id,'parent_id'=> $val->id,'active'=>'y'));
									}
									if ($menu) {
									$langStr .= CHtml::link("เลือกข้อสอบ [".$menu->getCountTest("pre")."] (".$value->language.")", array("Lesson/FormLesson","id"=>$menu->id,"type"=>"pre"), array("class"=>"btn btn-primary btn-icon")); 
									}
								}
						...
						array(
							...
							'value' => function($val) {
								...
								foreach ($lang as $key => $value) {
									...
									if ($menu) {
									$langStr .= CHtml::link("เลือกข้อสอบ [".$menu->getCountTest("post")."] (".$value->language.")", array("Lesson/FormLesson","id"=>$menu->id,"type"=>"post"), array("class"=>"btn btn-primary btn-icon")); 
									}
								}
								return '<div class="btn-group" role="group" aria-label="Basic example">'.$langStr.'</div>';
							},
