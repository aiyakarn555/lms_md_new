	...
	<div class="widget widget-tabs border-bottom-none">
		...
		<div class="widget-body">
			<div>
                ...
                <div class="row">
                    <div class="col-md-12 ">
                        <label><h4>ชื่อคอร์สเรียน : <?= $model->name_ms_teams ?></h4></label>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-12 ">
                        <label><h4>ชื่อบทเรียน : <?php $lessonExam = LessonMsTeams::model()->findByPk($lesson_id) ; echo $lessonExam->title; ?>
                ...
                <div class="row">
                    <div class="col-md-12 text-center">
                         <?php 
                           ...
                           $modelScoreExam = ScoreMsTeams::model()->find(array(
                            'condition' => 'user_id=:user_id AND lesson_teams_id=:lesson_teams_id AND type=:type AND active="y"',
                            'params' => array(':user_id' => $user_id,':lesson_teams_id' => $lesson_id,':type'=> 'post'),
                            'order' => 'score_id desc'));

                           $logQuesExam = LogquesMsTeams::model()->findAll(array(
                            'condition' => 'score_id=:score_id AND user_id=:user_id AND lesson_teams_id=:lesson_teams_id',
                            'params' => array(':score_id' => $modelScoreExam->score_id,':user_id'=>$user_id,':lesson_teams_id'=> $lesson_id))
                            );
                            $scoreExam = 0;
                            $totalScoreExam = 0;
                            $answerCheck = [];
                            foreach ($logQuesExam as $key => $value) {
                                ...
                                $answerQuestionExam = ChoiceMsTeams::model()->findAll(array(
                        ...
                        <table style="width:100%">
                            ...
                            <tbody>
                                ...
                                <?php   
                                    ...
                                    $criteria->compare("lesson_teams_id",$lesson_id);
                                    $criteria->compare("user_id", $user_id);
                                    $criteria->compare("type",'post');
                                    $criteria->compare("active","y");
                                    $criteria->order = 'score_number';
                                    $modelScore = ScoreMsTeams::model()->find($criteria);
                                    ...
                                    $logQues = LogquesMsTeams::model()->findAll(array(
                                        'condition' => 'score_id=:score_id AND user_id=:user_id AND lesson_teams_id=:lesson_teams_id AND test_type=:test_type',
                                        'params' => array(':score_id' => $modelScore->score_id,':user_id'=>$user_id,':lesson_teams_id'=> $lesson_id,':test_type'=> 'post')));
                      
                                    $i = 0;
                                    foreach ($logQues as $key => $value) {
                                        $question = QuestionMsTeams::model()->find(array(
                                        ...
                                        $answer = LogchoiceMsTeams::model()->findAll(array( 
                                        ...
                                        $answerQuestion = ChoiceMsTeams::model()->findAll(array(
                                            'condition'=>"ques_id=:ques_id and choice_answer='1'",
                                            'params' => array(':ques_id' => $value->ques_id)
                                        ));
