...
<?php if(isset($_GET['type_cou'])){ ?>
    <div class="widget" id="export-table33" >
        <div class="widget-head">
            <div class="widget-head">
                <h4 class="heading glyphicons show_thumbnails_with_lines"><i></i>ค้นหาโดยใช้ บทเรียน</h4>
            </div>
        </div> 
        <div class="widget-body" >
         <table class="table table-bordered table-striped" id="myTable">
            <thead>
                <tr>
					...
					<th  class="center text-white" rowspan="2">บทเรียน</th>
            ...
            <tbody>
					<?php
							...
							foreach($result as $i => $val) {
								$statusPre = ($val["lessonStatusPre"]=="y") ? "text-success":"text-danger" ;
								$statusPost = ($val["lessonStatusPost"]=="y") ? "text-success":"text-danger" ;
								...
								<tr>
									...
									<td class="center"><?= $val["lesson_title"] ?></td>
									<td class="center" >
                                            <b><span class="<?=$statusPre?>"><?= isset($val["lessonScorePre"]) ? $val["lessonScorePre"] :"-"  ?></span>
                                        /
                                        <?= isset($val["lessonTotalPre"]) ? $val["lessonTotalPre"] :"-" ?></b>
                                    </td>
									<td class="center" >
                                            <b><span class="<?=$statusPost?>"><?= isset($val["lessonScorePost"]) ? $val["lessonScorePost"] :"-"  ?></span>
                                        /
                                        <?= isset($val["lessonTotalPost"]) ? $val["lessonTotalPost"] :"-" ?></b>
                                    </td>
									<td class="center" style="width:20px;">
										<?=  CHtml::link("<i></i> แก้ไขผลการสอบ",array("/manageExamResult/UpdateLesson",
