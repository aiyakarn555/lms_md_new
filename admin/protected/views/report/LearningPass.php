                ...
                <dl class="dl-horizontal">
                    ...
                    <div class="form-group GenTest" style="display: none">
                        ...
                        <dd>
                            ...
                            <?php if( $_GET['course_id'] == null && $_GET['ms_teams_id'] == null  && $_GET['gen_id'] == null) { ?>
    ...
    if(isset($_GET['ms_teams_id']) && $_GET['ms_teams_id'] != "" ){
        ...
        if($type == "virtual"){
            ...
            if((isset($_GET['datestr']) && $_GET['datestr'] != "") && (isset($_GET['dateend']) && $_GET['dateend'] != "")){
                ...
                // $criteria->addCondition("DATE_FORMAT(pclog_date, '%Y-%m-%d') = '".$_GET['dateend']."'");
            ...
            foreach ($allUsersLogStartCourse as $valueByUser) {
                ...
                if($percent == 100){
                    ...
                    if(isset($_GET['course_codenum']) && $_GET['course_codenum'] != null){
                        $status = (!empty($PasscoursLog) && trim($PasscoursLog->cou_number) == trim($_GET['course_codenum'])) ? true : false;
                    }
                    if(isset($_GET['dateend']) && $_GET['dateend'] != ""){
                        if(!empty($PasscoursLog) ){
                            $status = (trim(date("Y-m-d", strtotime($PasscoursLog->pclog_date))) == trim($_GET['dateend'])) ? true : false;
                        }else{
                            $status = (trim(date("Y-m-d", strtotime($valueByUser->end_date))) == trim($_GET['dateend'])) ? true : false;
                        }
                    }
                    if($status){
                        $allUsersPassCourse[] =
                        array(
                            ...
                            "endDate"=>(!empty($PasscoursLog)) ? $PasscoursLog->pclog_date : $valueByUser->end_date,
                            "note"=>"-"
                        );
                    }
        ...
        $criteria = new CDbCriteria;
        $criteria->with = array('pro','gen','pass');
        ...
        if((isset($_GET['datestr']) && $_GET['datestr'] != "") && (isset($_GET['dateend']) && $_GET['dateend'] != "")){
                ...
                // $criteria->addCondition('t.start_date >= :date_str');
                // $criteria->params[':date_str'] = $_GET['datestr'];
                $criteria->addCondition(  "DATE_FORMAT(t.start_date, '%Y-%m-%d') = '".$_GET['datestr']."'");

                // $criteria->addCondition('t.end_date <= :date_end');
                // $criteria->params[':date_end'] = $_GET['dateend'];
                // $criteria->addCondition("DATE_FORMAT(t.end_date, '%Y-%m-%d') = '".$_GET['dateend']."' OR DATE_FORMAT(passcours_date, '%Y-%m-%d') = '".$_GET['dateend']."'");

            }else if(isset($_GET['datestr']) && $_GET['datestr'] != ""){

                // $criteria->addCondition('t.start_date>='.$_GET['datestr']);
                // $criteria->params[':date_str'] = $_GET['datestr'];
                $criteria->addCondition(  "DATE_FORMAT(t.start_date, '%Y-%m-%d') = '".$_GET['datestr']."'");
        ...
        if(isset($_GET['course_codenum']) && $_GET['course_codenum'] != "" ){
            $criteria->compare('pass.passcours_number', $_GET['course_codenum'], true);
        }

        $criteria->compare("t.active",'y');
        // $criteria->with = 'pass';
        // $criteria->addCondition('passcours_user = t.user_id');
        // $criteria->addCondition('passcours.gen_id = t.gen_id');
        // $criteria->order = 'passcours_number DESC';
        $allUsersLogStartCourse = LogStartcourse::model()->findAll($criteria);
        foreach ($allUsersLogStartCourse as $valueByUser) {
            $can_print_cer = 2;
            $CourseSurvey = CourseTeacher::model()->findAllByAttributes(array('course_id' => $courseOnline->course_id));
            if ($CourseSurvey) { // มี แบบสอบถาม
                foreach ($CourseSurvey as $key => $value) {
                    $num_step++;
                    $passQuest = QQuestAns_course::model()->find(array(
                        'condition' => 'user_id = "' . $valueByUser->user_id . '" AND course_id ="' . $courseOnline->course_id . '"' . " AND gen_id='" . $valueByUser->gen_id . "'",
                    ));
            ...
            if($percent == 100 && $can_print_cer == 1){
                $status = true;
                ...
                if(isset($_GET['course_codenum']) && $_GET['course_codenum'] != null){
                    $status = (!empty($Passcours) && trim($Passcours->passcours_number) == trim($_GET['course_codenum'])) ? true : false;
                }

                if(isset($_GET['dateend']) && $_GET['dateend'] != ""){
                    if(!empty($Passcours) ){
                        $status = (trim(date("Y-m-d", strtotime($Passcours->passcours_date))) == trim($_GET['dateend'])) ? true : false;
                    }else{
                        $status = (trim(date("Y-m-d", strtotime($valueByUser->end_date))) == trim($_GET['dateend'])) ? true : false;
                    }
                }
