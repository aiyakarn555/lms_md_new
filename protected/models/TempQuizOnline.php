...
/**
 * This is the model class for table "temp_quiz_online".
 *
 * The followings are the available columns in table 'temp_quiz_online':
 * @property integer $id
 ...
 */
class TempQuizOnline extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('user_id, lesson_teams_id, group_id, ques_id, number, status', 'numerical', 'integerOnly'=>true),
			array('type', 'length', 'max'=>5),
			array('question, time_up,ans_id', 'length', 'max'=>255),
			array('time_start', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, user_id, type, lesson_teams_id, group_id, ques_id, number, ans_id, status, time_start, question, time_up, gen_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'quest' => array(self::BELONGS_TO, 'QuestionOnline','ques_id'),
		);
	}
