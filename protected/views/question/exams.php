...
<input type="hidden" id="typefinis" value="0">
 ...
 <input type="text" placeholder="enablefacedetect" name="enablefacedetect" id="enablefacedetect" hidden value="<?= $lesson->enablefacedetect ?>">
 <input type="text" placeholder="intervalfacedetect" name="intervalfacedetect" id="intervalfacedetect" hidden value="<?= $lesson->intervalfacedetect ?>" >
 <input type="text" name="PopupDetails" id="PopupDetail" hidden value="<?= $lesson->popup_detail ?>">
	...
	function save_ans(evnt) {
		...
		if (evnt == 'save') {
			...
			swal({
				...
				if (isConfirm) {
					$.ajax({
						url: "<?php echo Yii::app()->createUrl('question/index', array('gen' =>$_GET['gen'], 'type'=>$testType)); ?>",
						type: "POST",
						data: $("#question-form").serialize(),
						success: function(data) {
							if ($('#last_ques').val() == 1) {
								var url = '<?php echo Yii::app()->createUrl('question/exams_finish', array('id' => $lesson->id,'gen' =>$_GET['gen'], 'type' => $testType)); ?>';
								if (evnt == 'save') {
                ...
                success:function(response){
                	...
                	$.ajax({
                		...
                		success:function(response){
                			...
                			if(response == "pass"){
                				intervalPop = setInterval(function () { getPopup(); }, ((intervalfacedetect * 60) * 1000) + 1000);
                			}else{
