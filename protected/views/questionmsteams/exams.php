									...
									<div class="col-lg-8 col-md-12 col-sm-12 col-xs-12">
										<div class="form-group">
											...
											foreach ($choiceData as $key => $val_choice) {
												$choice = ChoiceMsTeams::model()->findByPk($val_choice);
												$checked = '';
											...
											foreach ($Type4Answer as $key => $val_1) {
												...
												$choice = ChoiceMsTeams::model()->findByPk($key);
...
<script>
	...
	function save_ans(evnt) {
		...
		if (evnt == 'save') {
			...
			swal({
				...
				if (isConfirm) {
					$.ajax({
						url: "<?php echo Yii::app()->createUrl("questionmsteams/index", array('type' => $testType)); ?>",
						type: "POST",
						data: $("#question-form").serialize(),
						success: function(data) {
							if ($('#last_ques').val() == 1) {
								var url = '<?php echo Yii::app()->createUrl('questionmsteams/exams_finish', array('id' => $lesson->id, 'type' => $testType)); ?>';
								if (evnt == 'save') {
	...
	function time_test_start(time_down , test = null) {
		...
		interval = setInterval(function() {
			...
			if (seconds == 0) {
				...
				$.ajax({
					url: "<?php echo Yii::app()->createUrl("questionmsteams/SaveTimeExam"); ?>",
					type: "POST",
					data: {
						lesson_id: <?= $lesson->id ?>,
						time: count
	...
	var vid = document.querySelector('video[id="videoCap"]');
		...
		function ajax_db2()
		{
			...
			$.ajax({
				url:"<?php echo Yii::app()->createUrl('questionmsteams/webcam');?>",
				data: data,
                type:"POST", 
