...
$state = Helpers::lib()->CheckPostTestAllMsTeams($lesson);
?>
<script>
...
<div class="container">
					...
					<table class="table-question table table-bordered">
						...
						<tbody>
							...
							foreach($temp_all as $index => $val):
								$Logchoice = LogchoiceMsTeams::model()->findAll(" score_id=:s AND lesson_teams_id=:q AND logchoice_answer = 1  AND user_id=:user_id AND ques_id=:ques_id", array(
								...
								endforeach;
								$logScore = ChoiceMsTeams::model()->findAll(array(
	...
	// 	Helpers::lib()->resetScoreMsTeams($lesson->id);
	// 	$linkNextStep = Yii::app()->createUrl('questionmsteams/Resetpost/', array('id' => $lesson->id,'course'=>$lesson->ms_teams_id));
	// }else{
	// 	$linkNextStep = Yii::app()->createUrl('virtualclassroom/detail/', array('id' => $lesson->ms_teams_id));
	// } 
	 ?>
