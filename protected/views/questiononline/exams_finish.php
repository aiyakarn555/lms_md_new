...
if($modelScore->score_past == 'y'){
	...
	$linkPass = Yii::app()->createUrl('virtualclassroom/detail/', array('id' => $lesson->ms_teams_id));
	$link = '#modal-pass';
} else {
...
$state = Helpers::lib()->CheckPostTestAllOnline($lesson);
?>
<script>
	...
	<div class="container">
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb breadcrumb-main">
				<li class="breadcrumb-item"><a href="<?php echo $this->createUrl('/virtualclassroom/index'); ?>"><?php echo $labelCourse->label_course; ?></a>
					<li class="breadcrumb-item active" aria-current="page"><?= $lesson->msteams->name_ms_teams; ?></li>
				</ol>
			</nav>
		</div> 
		<input type="hidden" id="typefinis" value="1">
			...
			<div class="col-sm-4">
				<aside>
					...
					<ul class="list-unstyled">
						...
						$logQues = LogquesOnline::model()->findAll(array(
...
<div class="container">
					...
					<table class="table-question table table-bordered">
						...
						<tbody>
							...
							foreach($temp_all as $index => $val):
								$Logchoice = LogchoiceOnline::model()->findAll(" score_id=:s AND lesson_teams_id=:q AND logchoice_answer = 1  AND user_id=:user_id AND ques_id=:ques_id", array(
								...
								endforeach;
								$logScore = ChoiceOnline::model()->findAll(array(
								...
								$question = QuestionOnline::model()->find(array(
	...
	if($modelScore->type=='post' && !$state){
		Helpers::lib()->resetScoreOnline($lesson->id);
		$linkNextStep = Yii::app()->createUrl('questiononline/Resetpost/', array('id' => $lesson->id,'course'=>$lesson->ms_teams_id));
	}else{
		$linkNextStep = Yii::app()->createUrl('examsonline/detail/', array('id' => $lesson->ms_teams_id));
	} 
	 ?>
