                ...
                <div class="mt-2 mb-4 pt-3 pb-4">
                  <div class="form-group text-center">
                    ...
                    <div class="col take-camera-desktop mt-1 mb-1">
                          <button id="capture-photo" class="btn btn-primary mx-2 font-size-20" type="button"> <i class="fas fa-camera"></i> ถ่ายภาพ</button>         
                          <button type="button" id="clear-photo" class="btn btn-secondary mx-2 font-size-20" style="display:none;"> <i class="fa fa-repeat"></i> ถ่ายใหม่</button>
                          <button type="button" id="next" class="btn btn-success mx-2 font-size-20" style="display:none;"> <i class="fa fa-arrow-right"></i> ถัดไป</button>
                          <button type="button" id="previous" class="btn btn-default mx-2" style="display:none;"> <i class="fa fa-arrow-left"></i> ย้อนกลับ</button>
                          <button  type="button" onclick="login()" class="btn btn-warning mx-2 font-size-20" id="submit-button" disabled="true" style="display:none;">
                  ...
                  <!-- <div class="mt-2 text-center">
                        <button  type="button" onclick="login()" class="take-camera-desktop take-photo btn btn-warning mb-2" id="submit-button" disabled="true" style="display:none;">
                                   <?= $langId == 1 ? "Confirm" : "ยืนยันภาพถ่าย" ?>
                        </button>
                  </div> -->
...
var localMediaStream = null;
  ...
  const login = () => {
    ...
    Swal.fire({
        ...
        if (result.value) {
          ...
          window.location.href = "<?php echo $this->createUrl('registration/condition'); ?>"+'/'+$("#UserId").val();
        }
    });
  }
  ...
  .catch((err) => {
   ...
   let i = 1;
    ...
    //    $("#clear-photo").prop('disabled', false); //Clear Caputrue enabled



       $("#capture-photo").prop('disabled', true);//Caputrue disabled
    ...
    $("#clear-photo").click(function() {
       $("#clear-photo").prop('disabled', true);   //Clear Caputrue disabled 
       $("#capture-photo").prop('disabled', false); //Caputrue enabled
       $(".preview-main").css("display", "none"); // Open Vdo 
       //clearImage Insert
    ...
    checkSubmitButton = () => {
         if(faceImgData.face_image_1 != "" && faceImgData.face_image_2 != "" && faceImgData.face_image_3 != ""){
            $("#submit-button").css('display', "");//submit button disabled
            $("#submit-button").prop('disabled', false);//submit button disabled
            dataURLtoFile(faceImgData.face_image_1,$("#UserId").val(),"#face_image_1");
            dataURLtoFile(faceImgData.face_image_2,$("#UserId").val(),"#face_image_2");
            dataURLtoFile(faceImgData.face_image_3,$("#UserId").val(),"#face_image_3");
        }else{
             $("#submit-button").css('display', "none");
             $("#submit-button").prop('disabled', true);//submit button disabled
        }
    }
